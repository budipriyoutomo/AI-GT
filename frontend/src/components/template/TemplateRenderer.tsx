...
function aspectRatio(cfg: ResolvedConfig, override?: string): string {
...
// Style teks (tanpa posisi) — dipakai standalone maupun di dalam group. fontFamily sudah
// final hasil resolver (precedence brand/AI/template) — renderer hanya membaca s.fontFamily.
...
// Tagline perusahaan (data company profile, mirip slot `logo`) — value sudah diresolve
// (tagline live atau placeholder template) oleh resolver; di sini murni menggambar.
function TaglineElement({ el, scheme }: { el: TemplateElement; scheme: ColorScheme }) {
  const style: CSSProperties = {
    position: "absolute",
    left: pct(el.x),
    top: pct(el.y),
...
// Gradient scrim sudah diadaptasi brand oleh resolver — di sini murni menggambar apa adanya.
  ...
  return (
    ...
    >
      ...
      {(cfg.elements ?? []).map((el, i) => {
        switch (el.type) {
          case "logo":
            return <LogoElement key={i} el={el} logoUrl={cfg.logoUrl} />;
          case "text":
